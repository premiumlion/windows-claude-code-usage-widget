...
- **Resize** — drag left/right edges to adjust width (200–500 px)
...
- **Font size controls** — increase/decrease font via settings menu
...
- **Persistent state** — position, width, collapse state, section visibility, font size all saved
- **Auto-refresh** — stats every 30 s, usage API every 2 min
- **Version check** — notifies when a newer Claude Code version is available
...
pythonw "Claude Code Widget/claude-usage-widget.pyw"

# Run with console (for debugging)
python "Claude Code Widget/claude-usage-widget.pyw"
```
...
3. Target: `pythonw "C:\path\to\Claude Code Widget\claude-usage-widget.pyw"` (or the built exe path)
...
Edit the constants at the top of `Claude Code Widget/claude-usage-widget.pyw`:
