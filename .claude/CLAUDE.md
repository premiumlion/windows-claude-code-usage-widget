...
Single-file Python/tkinter desktop widget. Always-on-top overlay showing Claude Code usage stats.

**All source files live in `Claude Code Widget/`** — the main file is `Claude Code Widget/claude-usage-widget.pyw`.

## Architecture

- **One file**: `Claude Code Widget/claude-usage-widget.pyw` — no modules, no dependencies beyond stdlib
- **Icon**: `Claude Code Widget/claude-widget.ico`
...
- `get_local_claude_version()` → runs `claude --version` to get installed version
- `get_latest_claude_version()` → fetches public GitHub changelog for latest version
...
sleep 2
cd "/c/Users/oddsm/OneDrive/PPCB Plugin 2025/PPCB - Claude Widget/Claude Code Widget"
pyinstaller --onefile --windowed --icon=claude-widget.ico --name="ClaudeUsageWidget" claude-usage-widget.pyw
powershell -Command "Start-Process 'C:\\Users\\oddsm\\OneDrive\\PPCB Plugin 2025\\PPCB - Claude Widget\\Claude Code Widget\\dist\\ClaudeUsageWidget.exe'"
```
